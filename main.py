...
class ClientNode:
    ...
    def wait(self):
        for proc in self.procs:
            ...
            print(proc.stdout.read(), end="")  # pyright:ignore
            print(proc.stderr.read(), end="")  # pyright:ignore
            proc.wait()
...
def main():
    ...
    parser.add_argument(
        "--duration", type=float, help="how long to run the test for", default=3.2
    )
    ...
    parser.add_argument(
        "--clients_per_node", type=int, help="Hosts per node", default=1
    )
    ...
    client_identities = [
        l.strip().split(" ")[0].strip() for l in open(args.hosts, "r").readlines()
    ]
    client_nodes = []
    ...
    for node in client_nodes:
        node.run_agave_client(
            ...
            clients=args.clients_per_node,
            bind_ip=node.host.IP(),
        )
...
def topology(client_identities, args):
    ...
    net.addLink(
        server, switch, delay="1ms", bw=1000, limit=2000000
    )  # server side (fast)
    client_nodes = []
