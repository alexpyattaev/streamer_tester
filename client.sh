...
ip link add veth_srv-${TARGET_IP} type veth peer name veth_cli-${TARGET_IP}
#ethtool -K veth_srv-${TARGET_IP} gro off gso off tso off
#ethtool -K veth_cli-${TARGET_IP} gro off gso off tso off
#ip link set dev veth_srv-${TARGET_IP} txqueuelen 100000
#ip link set dev veth_cli-${TARGET_IP} txqueuelen 100000

ip link set dev veth_cli-${TARGET_IP} netns client${TARGET}
...
$CLI ip a a 10.0.1.${TARGET_IP}/24 dev veth_cli-${TARGET_IP}
$CLI ip link set veth_cli-${TARGET_IP} up

echo "Set host ${TARGET} link quality"
echo "Set delay of ${DELAY_MS}ms, packet loss ${LOSS_PERCENT}%"
QUEUE=2000000
$SRV tc qdisc add dev veth_srv-${TARGET_IP} root handle 1: netem delay ${DELAY_MS}ms # limit ${QUEUE}
#$SRV tc qdisc add dev veth_srv-${TARGET_IP} parent 1: handle 2: netem loss ${LOSS_PERCENT} rate 1000Mbit

$CLI tc qdisc add dev veth_cli-${TARGET_IP} root handle 1: netem delay ${DELAY_MS}ms # limit ${QUEUE}
#$CLI tc qdisc add dev veth-${TARGET_IP} parent 1: handle 2: netem loss ${LOSS_PERCENT} rate 1000Mbit
echo "Connectivity check"
$CLI ping 10.0.1.1  -c 1 -W 1 >> /dev/null
