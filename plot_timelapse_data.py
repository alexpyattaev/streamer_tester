...
def main():
    ...
    for dir in directories:
        ...
        for csv_file in csv_files:
            latency = (csv_file.replace('.', ' ').replace('-', ' ').split()[-2])
        ...
        for bin_file in binary_files:
            latency = (bin_file.replace('.', ' ').replace('-', ' ').split()[-2])
            #print(f"file:{bin_file} latency:{latency}")
            latencies.append(latency)
        ...
        print("bin files map")
        for k,v in latencies_map_bin.items():
            print(f"Keay: {k} | Value: {v}")
        print(set(latencies))
        for latency in set(latencies):
            ...
            for csv_file in csv_files:
                ...
                host = csv_file.replace('.', ' ').replace('-', ' ').replace('/',' ').split()[2]
                print(f"CSV_FILE:{csv_file} | HOST:{host}")
            ...
            for binary_file in binary_files:
                ...
                host = binary_file.replace('.', ' ').replace('-', ' ').replace('/',' ').split()[2]
                if "server" in  binary_file:
                    timestamps = np.fromfile(path, dtype=server_record_dtype)['time'] / 1_000_000
                else:
                    timestamps = np.fromfile(os.path.join(binary_file), dtype=np.uint32) / 1_000_000
            ...
            plt.suptitle(f"Latency: {latency} Clients amount: {len(csv_data.keys())}", fontsize=20)
            for host in csv_data["host"].unique():
                host_csv_data = csv_data[csv_data["host"] == host]
                color = next(color_cycle_csv)
                ax1.plot(host_csv_data["time_stamp"] / 1_000_000, host_csv_data["udp_tx"], label=f"{host[0:6]}-udp_tx", linestyle='-', color=color)
                ax1.plot(host_csv_data["time_stamp"] / 1_000_000, host_csv_data["udp_rx"], label=f"{host[0:6]}-udp_rx", linestyle='--', color=color)
            ...
            for host in binary_data["host"].unique():
                ...
                ax2.plot(host_binary_data["timeline"], host_binary_data["TPS"], label=f"{host[0:6]}", linestyle='-', color=color)
